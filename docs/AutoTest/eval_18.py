...
def validate_task_18(result=None, device_id=None, backup_dir=None):
    ...
    try:
        ...
        # 2. 验证逻辑：检查返回消息中是否包含"已关闭"或"关闭"
        if '已关闭' in final_msg or '关闭' in final_msg:
            print(f"✓ 验证成功: 返回消息包含关闭状态 - {final_msg}")
            return True
        else:
            print(f"❌ 验证失败: 返回消息中未包含关闭状态 - {final_msg}")
            return False

    except subprocess.TimeoutExpired:
        print("⚠️ ADB命令超时，回退验证")
        return '已关闭' in final_msg or '关闭' in final_msg
