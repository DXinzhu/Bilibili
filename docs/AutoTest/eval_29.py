...
def validate_task_29(result=None, device_id=None, backup_dir=None):
    ...
    try:
        ...
        if result_data.returncode != 0 or not result_data.stdout:
            print("Cannot read live streams data, fallback validation")
            pattern = r'(?<!\d)8623(?!\d)'
        ...
        if len(live_streams) < 2:
            print(f"Live streams count < 2 (current: {len(live_streams)}), fallback validation")
            pattern = r'(?<!\d)8623(?!\d)'
            return bool(re.search(pattern, final_msg))
        ...
        pattern = rf'(?<!\d){total_viewers}(?!\d)'
        if re.search(pattern, final_msg):
            print(f"Validation SUCCESS: Total viewers = {total_viewers}")
            return True
        else:
            print(f"Validation FAILED: Expected={total_viewers}, Actual={final_msg}")
            return False
